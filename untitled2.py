...
def anl_tweets(lst, title='Tweets Sentiment', engl=True ):
    ...
    if(pos!=0 or neg!=0 or neu!=0):               
        ...
        if(pos1!=0 and neg1!=0 and neu1!=0):
            rmse=[abs(pos1-values1[0])/pos1*100,abs(neg1-values1[1])/neg1*100,abs(neu1-values1[2])/neu1*100]
        else:
            a4=0;b4=0;c4=0
            if(pos1!=0):
                a4=abs(pos1-values1[0])/pos1*100
            if(neg1!=0):
                b4=abs(neg1-values1[0])/neg1*100
            if(neu1!=0):
                c4=abs(neu1-values1[2])/neu1*100        
            rmse=[a4,b4,c4]

    else:
        pos1=0;neg1=0;neu1=0;    
        rmse=[0,0,0]
    print(rmse)
...
def show_entry_fields():
    ...
    words=e1.get().split("delimiter")
    for word in words:
        filter.append(word)
    print(filter)    
    w=tk.Label(root, text="Searching for {}".format(e1.get()))
    w.pack()
    while(stop==1):
